...
class PatchApplier
{
    ...
    public function applyPatches(PackageInterface $package, array $patchesQueue)
    {
        ...
        foreach ($patchesQueue as $source => $info) {
            ...
            $this->outputPatchFileInfo($source, $info);

            $loggerIndentation = $this->logger->push();

            $this->outputPatchDescription($info);
    ...
    private function outputPatchFileInfo($source, $info)
    {
        ...
        $isOwnerUnknown = !$info[PatchDefinition::OWNER]
            || $info[PatchDefinition::OWNER] === PatchDefinition::OWNER_UNKNOWN;

        if ($isOwnerUnknown) {
            $this->logger->writeRaw('%s%s', array($source, $label));
        } else {
            $this->logger->writeRaw(
                '<info>%s</info>: %s%s',
                array($info[PatchDefinition::OWNER], $source, $label)
            );
        }
    }

    private function outputPatchDescription($info)
    {
